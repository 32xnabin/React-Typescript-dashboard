...
import { BlueHeader, WhiteLabel, HorDiv } from './Cases.style';
import { NewButton } from './Cases.style';
...
const Cases: React.FC<IProps> = ({
  ...
  const handleSelectAllCases = (event: ChangeEvent<HTMLInputElement>): void => {
    setSelectedCases(event.target.checked ? cases.map((item) => item.id) : []);
  };
  ...
  const filteredCases = performFilter(cases, personName);
                      ...
                      return (
                        <TableRow hover key={item.id} selected={isCaseselected}>
                          ...
                          <TableCell>
                            ...
                            >
                              {item.added_date
                                ? moment(new Date(item.added_date)).format(
                                    'DD/MM/YYYY'
                                  )
