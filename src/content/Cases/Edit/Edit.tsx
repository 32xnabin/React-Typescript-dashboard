...
import { getCaseById, uploadImage } from '../../../services/cases';
...
interface IProps {
  cases?: Myboscase[];
  updateCase?: (caseObj: Myboscase) => void;
  fetchCases?: () => void;
}

const Edit: React.FC<IProps> = ({ updateCase, fetchCases, cases }) => {
  ...
  const prefillForm = (item: Myboscase) => {
    ...
    if (item.contacts) {
      setContacts(prepareMultiSelect(item.contacts));
    }
    if (item.asset) {
    ...
    setValue('apartment', item.apartment);
    if (item.job_area) {
      setJobArea(JSON.parse(item.job_area));
  ...
  const onSubmit = (data: Myboscase) => {
    if (assignedTo.length > 0) {
      ...
      data['add_to_report'] = false;
      data['duplicate_case'] = false;
